...
CREATE TABLE users (
    ...
    -- Bumped to invalidate every outstanding session (e.g. on password reset). The BFF
    -- carries this as a session claim; the API rejects tokens behind the current value.
...
-- Single-use, time-boxed password reset tokens (raw token emailed; only the hash stored).
